    ...
    <div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-card border border-brand-gray-light bg-white p-5">
          <p className="text-sm text-brand-gray">Total Orders</p>
          <p className="mt-1 text-2xl font-bold text-brand-ink">{stats.orderCount}</p>
        </div>
        <div className="rounded-card border border-brand-gray-light bg-white p-5">
          <p className="text-sm text-brand-gray">Revenue (all-time)</p>
          <p className="mt-1 text-2xl font-bold text-brand-ink">{formatPrice(stats.revenue, "₮")}</p>
        </div>
        <div className="rounded-card border border-brand-gray-light bg-white p-5">
          <p className="text-sm text-brand-gray">Products</p>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-card border border-brand-gray-light bg-white p-5">
          ...
          {stats.recentOrders.length === 0 ? (
            <p className="mt-3 text-sm text-brand-gray">No orders yet.</p>
          ) : (
            <ul className="mt-3 divide-y divide-brand-gray-light">
              {stats.recentOrders.map((o) => (
                <li key={o.id} className="flex items-center justify-between py-2 text-sm">
                  ...
                  <span className="text-brand-gray">{o.customer_name}</span>
                  <span className="rounded-full bg-brand-cream px-2 py-0.5 text-xs">{o.status}</span>
